...
Questo documento raccoglie tutto il materiale della lezione. Parte del materiale didattico è stato adattato dalle slide del Prof. [Massimiliano Pastore](https://dpss.psy.unipd.it/~massimiliano.pastore/index.html) (a.a. 2024/2025).
...
Ogni segmento rosso è un errore $\epsilon_i = y_i - (\beta_0 + \beta_1 x_i)$: di
quanto il dato osservato si discosta dal valore atteso sulla retta. (A rigore
l'errore si misura dalla retta *vera*; quando la retta viene *stimata* dai dati,
quella distanza si chiama **residuo** — è la stessa idea, e la calcoleremo nella
@sec-scarti.)

Le cinque assunzioni dicono, ciascuna, qualcosa su **come devono comportarsi
questi segmenti**. Accanto a ognuna trovate un grafico con dati simulati: a
sinistra l'assunzione rispettata, a destra violata. 
...
dove le $D_j$ sono **variabili dummy**: nuove colonne, costruite dalla variabile
categoriale, che valgono **1** se l'osservazione appartiene a una certa categoria
e **0** altrimenti. Una categoria — detta **di riferimento** — non riceve una
colonna propria: corrisponde a *tutte le dummy uguali a 0*. Ecco perché per $k$
categorie bastano $k-1$ dummy.
...
cioè la covarianza tra $x$ e $y$ divisa per la varianza di $x$.
...
- **`t value` e `Pr(>|t|)`**: la statistica $t$ (stima / errore standard) e il
  relativo p-value per l'ipotesi nulla $\beta = 0$, calcolato sulla
  distribuzione $t$ con i gradi di libertà residui (il box nella @sec-bande
  spiega da dove arriva la $t$). Qui il test su `mom_iq` è
  significativo.
...
- **`F-statistic`**: il test **globale** del modello contro il modello nullo
  ...
  sono zero", alternativa "almeno uno è diverso da zero". Serve da controllo
  d'insieme prima di guardare i singoli test $t$: facendo molti test, qualche
  p-value piccolo può saltare fuori anche solo per caso.
...
a–b. I p-value coincidono con quelli del `summary()` (4.36 dà 1.63e-05 — con
la normale sarebbe stato 1.3e-05: sono le code più pesanti della $t$ all'opera;
per 10.42 il valore è così piccolo che R lo riporta come `< 2.2e-16`, il limite
della precisione di stampa).
...
p-value coincidono (a meno degli arrotondamenti di $t$ e $F$ letti
dall'output).
:::
...
L'interpretazione, con una dummy, è in termini di **medie di gruppo** (è
esattamente la logica vista in @sec-codifica):
...
i dati. Per questo $\mathcal{L}(\mu)$ non è una distribuzione di probabilità (non
integra a 1): è una funzione che *ordina* i valori dei parametri per
compatibilità con il campione.
:::

## A che cosa serve

1. **Ottimizzare.** Cercare il valore dei parametri che la massimizza: è la
   **stima di massima verosimiglianza** (*Maximum Likelihood Estimation*, MLE),
...
\log\mathcal{L} = -\frac{n}{2}\log(2\pi\sigma^2) - \frac{\sum_i (y_i - \mu_i)^2}{2\sigma^2}
$$

Guardate il numeratore dell'ultimo termine: è la **devianza residua**
(@sec-devianze). Massimizzare la verosimiglianza rispetto ai $\beta$ significa
quindi *minimizzare* $\sum_i (y_i - \hat{y}_i)^2$: sotto l'assunzione di
normalità, **minimi quadrati e massima verosimiglianza scelgono la stessa
retta**.

Verifichiamolo: ricalcoliamo "a mano" la log-verosimiglianza di `m1` come somma
...
Identiche. (Unica accortezza: la stima di massima verosimiglianza di $\sigma$ usa
$n$ al denominatore, non gli $n - k - 1$ del *Residual standard error* — perché?)

::: {.callout-note collapse="true"}
## Perché due denominatori diversi per la stessa $\sigma$?

Sono due stimatori diversi: $\hat\sigma^2_{\text{ML}} = \text{SS}_E/n$ (massima
verosimiglianza) e $\hat\sigma^2 = \text{SS}_E/(n-k-1)$ (*Residual standard error*).

**Perché differiscono.** La retta di `lm()` è quella che *minimizza* $\text{SS}_E$:
si incolla a *questo* campione meglio della retta vera. Quindi i residui sono
sistematicamente più piccoli dei veri errori, e $\text{SS}_E/n$ **sottostima**
$\sigma^2$. L'esempio estremo: con una regressione semplice e **solo 2 punti** la
retta ci passa esattamente sopra, $\text{SS}_E = 0$, e $\text{SS}_E/n = 0$ direbbe
"errore nullo" — assurdo (con 2 punti non sai nulla sulla dispersione attorno alla
retta). Dividendo per $n-k-1 = 0$ si ottiene $0/0$, indefinito: la risposta giusta,
"non stimabile".

**La correzione.** In media $E[\text{SS}_E] = (n-k-1)\,\sigma^2$, quindi
$\text{SS}_E/(n-k-1)$ è **non distorto**, mentre $\text{SS}_E/n$ è troppo piccolo
del fattore $(n-k-1)/n$. Quel $n-k-1$ sono i **gradi di libertà residui**: i
residui obbediscono a $k+1$ vincoli (le equazioni normali), quindi solo $n-k-1$
sono liberi — esattamente come la varianza campionaria divide per $n-1$, avendo
speso un grado di libertà per stimare la media.

**Perché allora la verosimiglianza usa $n$?** Perché la massima verosimiglianza
cerca il valore che rende più probabili i dati: il massimo rispetto a $\sigma^2$
cade *esattamente* in $\text{SS}_E/n$ — non è pensata per essere non distorta. Per
questo `logLik()`, AIC e BIC usano $n$, mentre `summary()` (Residual standard
error), errori standard, test $t$ e bande usano $n-k-1$. Con $n$ grande la
differenza è trascurabile (qui 434 contro 432).

Verifichiamolo: simuliamo tanti campioni con $\sigma$ noto e confrontiamo le medie
delle due stime.

```{r}
#| label: ml-vs-corretta
set.seed(1)
stime <- replicate(5000, {
  x <- 1:8                                  # n = 8, un predittore (k = 1)
  y <- 2 + 0.5 * x + rnorm(8, 0, sd = 4)    # sigma vero = 4, quindi sigma^2 = 16
  e <- residuals(lm(y ~ x))
  c(ML = sum(e^2) / 8, corretta = sum(e^2) / (8 - 1 - 1))   # /n   vs   /(n-k-1)
})
rowMeans(stime)        # media delle stime di sigma^2 (vero = 16)
```

La stima ML centra in media un valore *più basso* del vero $\sigma^2 = 16$ (circa
$16 \times 6/8 = 12$), mentre quella corretta lo centra in pieno.
:::

::: {.callout-important}
**L'assunzione di normalità non è un
dettaglio tecnico — è lei a definire la funzione di verosimiglianza del
modello**. Assumere la normalità significa decidere che i dati vanno valutati
con campane normali centrate sulla retta: è questa la verosimiglianza che
...
b. cercate il massimo della funzione di verosimiglianza su una griglia fine di
   valori di $\mu$: coincide con `mean(y)`?
...
- **`F` e `Pr(>F)`** — il test $F$ chiede se quel calo è *più grande di quanto ci
  ...
  aggiunto) con la varianza residua del modello. Un p-value piccolo dice "vale la
  pena aggiungere quel termine".

Così la **riga 2** (aggiungere `mom_iq` al nullo) abbatte la devianza di 36249,
$F$ enorme, $p < 2.2\text{e-}16$: il QI è fortemente predittivo. L'**ultima riga**
(aggiungere l'interazione) fa calare la devianza di 2878, $p = 0.003$: anche
l'interazione aggiunge qualcosa di significativo.
